...
## 3단계 — Skill로 컬럼 추가·벤치마크 절차 캡슐화

**사용한 기능**: `.claude/skills/<name>/SKILL.md`. `transaction-column`(컬럼 추가 규칙)과 `render-benchmark`(렌더링 벤치마크 절차) 두 개를 만들었다.

**CLAUDE.md / Skill / Subagent / Hook — 네 가지의 역할 차이**: CLAUDE.md는 세션 내내 항상 켜져 있는 상시 배경지식이고, Skill은 지금 하는 작업이 맞을 때만 그 순간 로드되는 절차서다. Subagent는 별도 컨텍스트에서 독립적으로 실행되고 결과만 돌려주는 위임 단위이고, Hook은 모델의 판단을 거치지 않고 이벤트마다 하네스가 강제로 실행하는 검증기다(2단계).

**넷이 겹치지 않게 쓰인 사례**: "코드를 고친 뒤 lint를 돌려라"는 이미 Hook이 강제하고 있으므로 `transaction-column` Skill에는 그 내용을 다시 적지 않았다. 대신 "새 컬럼에 `size`를 빠뜨리면 `table-layout: fixed` 모드에서만 조용히 컬럼 폭이 깨진다"처럼, Hook은 실행 여부만 강제할 뿐 절대 대신 잡아줄 수 없는 지식 — 코드를 읽거나 직접 겪어봐야 아는 것 — 만 Skill에 담았다. Hook이 이미 보장하는 것을 Skill에 또 적으면 중복이고, Skill에 담아야 할 걸 Hook으로 만들려 하면 애초에 표현이 안 된다(Hook은 "어떻게"를 설명하지 못한다).

**왜 이 두 개를 Skill로 만들었는지**: 둘 다 코드를 한 번 읽는다고 바로 드러나지 않고 직접 겪어봐야 알게 되는 함정이었다 — `size`를 빠뜨리면 fixed 모드에서만 깨지는 문제, dev 서버로 측정하면 안 되는 이유 같은 것들. 그리고 컬럼 추가와 성능 측정 둘 다 오늘 한 번으로 끝나는 일이 아니라 다음에 같은 작업을 할 때 또 반복될 지식이라, 그때마다 처음부터 다시 겪지 않도록 Skill로 캡슐화해뒀다.
