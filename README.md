...
### 1. 실시간 환율 기반 스마트 환전
- 지연 없는 정보 제공: 외부 API 연동과 지능형 캐싱 전략을 통해, 사용자는 기다림 없이 항상 최신의 환율 정보를 확인하고 즉시 환전할 수 있습니다.
- 최적의 환전 타이밍: 실시간 데이터 갱신을 통해 가장 유리한 시점에 외화를 확보할 수 있는 환경을 제공합니다.

### 2. 멀티 커런시(Multi-Currency) 지갑 & 송금
- 통합 자산 관리: 여러 국가의 화폐를 하나의 지갑에서 간편하게 관리하고 잔액을 조회할 수 있습니다.
- 끊김 없는 거래: 사용자 간의 송금 프로세스를 모듈화하여, 어떤 통화로든 빠르고 정확하게 자금을 주고받을 수 있는 ACID 트랜잭션 환경을 구축했습니다.

### 3. 보안을 최우선으로 하는 카드 서비스
- 안전한 카드 발급: 실제 금융권 수준의 암호화 기술(KMS, RSA, AES)을 적용하여 실물 카드를 발급하고 관리합니다.
- 민감 정보 철벽 보호: 카드 번호와 CVC 등 핵심 정보는 이중 암호화되어 저장되며, 조회 시에도 철저한 보안 검증을 거칩니다.

### 4. 믿을 수 있는 거래 모니터링
- 실시간 알림 서비스: 환전이나 송금 실패 등 특이사항 발생 시 Slack을 통해 사용자에게 즉시 알림을 발송하여 거래 가시성을 높였습니다.
- 장애 복구 시스템: 예상치 못한 시스템 장애 상황에서도 거래 기록이 유실되지 않도록 견고한 복구 로직이 설계되어 있습니다.
