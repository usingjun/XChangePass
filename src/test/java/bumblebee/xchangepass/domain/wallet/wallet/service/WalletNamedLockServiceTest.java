...
class WalletNamedLockServiceTest {
    ...
    void testChargeWallet() {
        lockWalletFacade.charge(sender.getUserId(), new WalletInOutRequest(CHARGE_AMOUNT, CURRENCY, CURRENCY, LocalDateTime.now()));
        ...
        for (int i = 0; i < concurrentUsers; i++) {
            executorService.submit(() -> {
                try {
                    ...
                    lockWalletFacade.transfer(senderId, transferRequest);
                } catch (Exception e) {
    ...
    void eitherTransferOrWithdrawalFailsDuringConcurrentExecution() throws Exception {
        lockWalletFacade.charge(sender.getUserId(), new WalletInOutRequest(
        ...
        Future<?> withdrawFuture = executorService.submit(() -> {
            try {
                ...
                lockWalletFacade.withdrawal(sender.getUserId(), withdrawRequest);
    ...
    void chargeSucceedsAndTransferFailsOnConcurrentRequest() throws Exception {
        ...
        Future<?> chargeFuture = executorService.submit(() -> {
            try {
                ...
                lockWalletFacade.charge(sender.getUserId(), chargeRequest);
